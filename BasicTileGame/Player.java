...
public class Player extends Character {
    ...
    public Player(TileMap tm) {
         ...
         try {
             ...
             idleSprites[0] = ImageIO.read(new File("graphics/kirbyidle.gif"));
             jumpingSprites[0] = ImageIO.read(new File("graphics/kirbyjump.gif"));
             fallingSprites[0] = ImageIO.read(new File("graphics/kirbyfall.gif"));

             BufferedImage image = ImageIO.read(new File("graphics/kirbywalk.gif"));
    ...
    private void calculateDoors(double x, double y) {
        int leftTile = tileMap.getColTile((int) (x - width / 2));
        int rightTile = tileMap.getColTile((int) (x + width / 2) - 1);
        int topTile = tileMap.getRowTile((int) (y - height / 2));
        int bottomTile = tileMap.getRowTile((int) (y + height / 2) - 1);
        topLeft = tileMap.isDoor(topTile, leftTile);
        topRight = tileMap.isDoor(topTile, rightTile);
        bottomLeft = tileMap.isDoor(bottomTile, leftTile);
        bottomRight = tileMap.isDoor(bottomTile, rightTile);
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public void update(ArrayList<Character> characters) {
        ...
        int playerCenterX = (int)((getWidth() / 2) + x);
        int playerCenterY = (int)((getHeight() / 2) + y);


        for(int i = 0; i < characters.size(); i++) {
        int monsterCenterX = (int)((characters.get(i).getWidth() / 2) + characters.get(i).getx());
        int monsterCenterY = (int)((characters.get(i).getWidth() / 2) + characters.get(i).gety());


        if(Math.abs(playerCenterX - monsterCenterX) < 5 & Math.abs(playerCenterY - monsterCenterY) < 10) {


           if(characters.get(i).getx() <= x) {
               health = health - 10;
               dx += maxSpeed;
               //characters.remove(i);
      ...
      //         dx -= maxSpeed;
        ...
        //move the map to adjust to player
